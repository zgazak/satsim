...
from satsim.geometry.draw import (
    gen_line,
    gen_line_from_endpoints,
    gen_curve_from_points,
)
...
def image_generator(
    ...
    # save annotation frame that contains instance segmentation by percentage of noise, star, RSO
    if "save_percent_annotation_frame" not in ssp["sim"]:
        ssp["sim"]["save_percent_annotation_frame"] = True
    ...
    if star_mode == "bins":
        ...
        star_dec = (
            ssp["geometry"]["stars"]["dec"] if "dec" in ssp["geometry"]["stars"] else 0
        )
        star_rot = (
            ssp["geometry"]["stars"]["rotation"]
            if "rotation" in ssp["geometry"]["stars"]
            else 0
        )
    ...
    if ssp["geometry"]["stars"]["motion"]["mode"] == "affine":
        ...
        star_tran_os = [
            ssp["geometry"]["stars"]["motion"]["translation"][0],
            ssp["geometry"]["stars"]["motion"]["translation"][1],
        ]
        ...
        star_rot_rate = ssp["geometry"]["stars"]["motion"]["rotation"]
        star_vel_angle = (
            ssp["geometry"]["stars"]["motion"]["translation"][0] * math.pi / 180
        )
        star_vel_mag = ssp["geometry"]["stars"]["motion"]["translation"][1]
        star_tran_os = [
            star_vel_mag * math.sin(star_vel_angle),
            star_vel_mag * math.cos(star_vel_angle),
        ]
        star_tran_os[0] = star_tran_os[0] / y_to_pix * s_osf
        star_tran_os[1] = star_tran_os[1] / x_to_pix * s_osf
    ...
    if "pad" in ssp["geometry"]["stars"]:
        ...
        star_pad = 1.2
    star_bounds = [
        [-star_pad * h_fpa_pad_os, (star_pad + 1) * h_fpa_pad_os],
        [-star_pad * w_fpa_pad_os, (star_pad + 1) * w_fpa_pad_os],
        [0, 0],
    ]
    ...
    bg = (
        mv_to_pe(zeropoint, ssp["background"]["galactic"])
        * (y_ifov * 3600 * x_ifov * 3600)
        * exposure_time
    )

    if "stray" in ssp["background"]:
        if (
            isinstance(ssp["background"]["stray"], dict)
            and "mode" in ssp["background"]["stray"]
            and ssp["background"]["stray"]["mode"] == "none"
        ):
    ...
    while num_sets <= 0 or set_number < num_sets:
        ...
        def calculate_az_el(tt):
            ...
            el = interp_degrees(
                ttt, t0.tt, t2.tt, track_el[0], track_el[1], normalize_360=False
            )
        ...
        def calculate_star_position_and_motion(t_start, t_end, star_rot, track_mode):
            ...
            if track_mode == "rate":
                track_target = [track]
                star_ra0, star_dec0, dis0, az0, el0, los0 = get_los(
                    observer, track, t_start
                )
                star_ra1, star_dec1, dis1, az1, el1, los1 = get_los(
                    observer, track, t_end
                )
            elif track_mode == "fixed":
                track_target = []
                star_ra0, star_dec0, dis0, az0, el0, los0 = get_los_azel(
                    observer, az[0], el[0], t_start
                )
                star_ra1, star_dec1, dis1, az1, el1, los1 = get_los_azel(
                    observer, az[1], el[1], t_end
                )
            else:
                track_target = [track]
            ...
            star_tran_os = [
                -drr,
                -dcc,
            ]  # stars move in the opposite direction of target
            star_rot_rate = 0  # TODO

            astrometrics["ra"] = mean_degrees(star_ra0, star_ra1)
            astrometrics["dec"] = (star_dec0 + star_dec1) / 2
            astrometrics["ra_rate"] = (
                diff_degrees(star_ra1, star_ra0) / exposure_time * 3600
            )
        ...
        if "site" in ssp["geometry"]:
            ...
            observer = create_topocentric(
                ssp["geometry"]["site"]["lat"], ssp["geometry"]["site"]["lon"]
            )
            ...
            if "tle" in ssp["geometry"]["site"]["track"]:
                track = create_sgp4(
                    ssp["geometry"]["site"]["track"]["tle"][0],
                    ssp["geometry"]["site"]["track"]["tle"][1],
                )
            elif "tle1" in ssp["geometry"]["site"]["track"]:
                track = create_sgp4(
                    ssp["geometry"]["site"]["track"]["tle1"],
                    ssp["geometry"]["site"]["track"]["tle2"],
                )
            elif "epoch" in ssp["geometry"]["site"]["track"]:
                track_epoch = time.utc_from_list_or_scalar(
                    ssp["geometry"]["site"]["track"]["epoch"]
                )
                track = create_twobody(
                    np.array(ssp["geometry"]["site"]["track"]["position"]) * u.km,
                    np.array(ssp["geometry"]["site"]["track"]["velocity"]) * u.km / u.s,
                    track_epoch,
                )
            else:
                track = None
            ...
            if "time" in ssp["geometry"]["site"]:
                logger.warning(
                    "geometry.site.time is deprecated. Use geometry.time instead."
                )
            ...
            (
                ...
                star_tran_os,
                star_rot_rate,
            ) = calculate_star_position_and_motion(t0, t1, star_rot, track_mode)
        else:
            track_mode = None
        ...
        if ssp["sim"]["temporal_osf"] == "auto":
            if star_mode != "none":
                ...
                t_osf = tf.cast(
                    max(
                        [
                            tf.sqrt(rrr[0] * rrr[0] + ccc[0] * ccc[0]),
                            tf.sqrt(rrr[1] * rrr[1] + ccc[1] * ccc[1]),
                        ]
                    )
                    + 1,
                    tf.int32,
                ...
                logger.debug(
                    "Auto temporal oversample factor set to {}.".format(t_osf.numpy())
                )
            else:
                t_osf = 1
        ...
        if not isinstance(ssp["fpa"]["psf"], dict):  # loaded from config
            ...
            psf_os = gen_from_poppy_configuration(
                h_sub_pad_os / s_osf,
                w_sub_pad_os / s_osf,
                y_ifov,
                x_ifov,
            ...
            for i, o in enumerate(obs):
                ...
                if "events" in o:
                    if "create" in o["events"]:
                        ts_start_ob = time.utc_from_list_or_scalar(
                            o["events"]["create"], default_t=tt
                        )
                        if ts_end.tt <= ts_start_ob.tt:
                            continue
                    if "delete" in o["events"]:
                        ts_end_ob = time.utc_from_list_or_scalar(
                            o["events"]["delete"], default_t=tt
                        )
                ...
                if "m_v" in o:
                    ...
                    pe_func = (
                        (lambda x, t: x)
                        if not callable(o["pe"])
                        else (lambda x, t: o["pe"](x, t) * _delta_t(t))
                    )

                if o["mode"] == "line":
                    ovrc = [
                        o["velocity"][0] / y_to_pix * s_osf,
                        o["velocity"][1] / x_to_pix * s_osf,
                    ]
                    epoch = o["epoch"] if "epoch" in o else 0
                    (orr, occ, opp, ott) = gen_line(
                        h_fpa_os,
                        w_fpa_os,
                        o["origin"],
                        ovrc,
                    ...
                    ovrc = [
                        o["velocity"][1] * math.sin(o_vel_angle),
                        o["velocity"][1] * math.cos(o_vel_angle),
                    ]
                ...
                elif o["mode"] == "tle" or o["mode"] == "twobody" or o["mode"] == "gc":
                    if obs_cache[i] is None or updated:
                        if o["mode"] == "tle":
                            ...
                            ts_epoch = time.utc_from_list_or_scalar(
                                o["epoch"], default_t=tt
                            )
                            if "az" in o:
                                obs_cache[i] = [
                                    GreatCircle(
                            ...
                            obs_cache[i] = [
                                create_twobody(
                                    np.array(o["position"]) * u.km,
                                    np.array(o["velocity"]) * u.km / u.s,
                                    ts_epoch,
                                )
                            ]
                    ...
                    try:
                        ...
                        logger.warning(
                            "Error propagating target. Possibly unstable element set: {}".format(
                                o
                            )
                        )
                        continue
                    ...
                    if (
                        len(rr0) > 0
                        and not math.isnan(rr0[0])
                        and not math.isnan(cc0[0])
                        ...
                        if ssp["sim"]["num_target_samples"] == 2:
                            (orr, occ, opp, ott) = gen_line_from_endpoints(
                                rr0[0], cc0[0], rr2[0], cc2[0], ope, t_start, t_end
                            )
                        else:
                            (orr, occ, opp, ott) = gen_curve_from_points(
                                rr0[0],
                                cc0[0],
                                rr1[0],
                                cc1[0],
                ...
                logger.debug(
                    "Average brightness for target: {:.2f} mv, {:.2f} pix.".format(
                        avg_mv, len(opp) / s_osf
                    )
                )
                ...
                if "model" in o:
                    if render_mode != "piecewise":
                        if not callable(o["model"]):
                            patch = (
                                load_sprite_from_file(filename=o["model"]["filename"])
                                if "mode" in o["model"]
                                else o["model"]
                            )
                            fpa_os_clear = tf.zeros(
                                [h_fpa_pad_os, w_fpa_pad_os], tf.float32
                            )
                            fpa_os_w_targets = add_patch(
                                fpa_os_clear,
                                orr,
                                occ,
                                opp,
                        ...
                        logger.warning(
                            "Sprite models not supported for piecewise rendering."
                        )
                else:
