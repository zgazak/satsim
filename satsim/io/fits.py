...
def save(
    ...
    rd = SkyCoord(
        ra=get_or_default("ra", 0) * u.degree,
        dec=get_or_default("dec", 0) * u.degree,
        frame="icrs",
    )
    ...
    orchcomm = "{}#[{}:{}]@[{}]%[filter wheel disabled by user]".format(
        get_or_default("track_mode"),
        get_or_default("frame_num"),
        get_or_default("num_frames"),
        get_or_default("site", "sim"),
    )
    ...
    if seg_map is not None:
        seg_map_cpy = np.copy(seg_map)
        hdu_map = fits.ImageHDU(seg_map_cpy)
        hdr_map = hdu_map.header

        fits_file = fits.HDUList([hdu, hdu_map])
